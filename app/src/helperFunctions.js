...
const readUsergroupsFromCleanSlate = async ({ app, usergroups }) => {
  ...
  if (!usersOkay) {
    ugs.usergroups.forEach(async (ug) => {
      ...
      if (!res) {
        console.log(
          `Something went awry when trying to insert usergroup users for usergroup ${ug.id}`,
        );
      }
    });
  }
...
const formatUserIdList = (userIdList, userFormatter) => {
  return userIdList.map((user) => userFormatter(user)).sort(Intl.Collator().compare);
};

/**
 * Formats a JS date object into a Slack mrkdwn formatted string.
 * Will automatically show the time in the user's time zone.
 * @param {Object} date JavaScript date object
 * @returns Date string in Slack mrkdwn format
 */
const formatDates = (date) => {
  const dateInSeconds = date.getTime().toString().slice(0, -3);
  const mrkdwnDate = `<!date^${dateInSeconds}^{date_num} {time_secs}|${date}>`;
  return mrkdwnDate;
};
