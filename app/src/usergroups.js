...
const generateMentionString = (slack_usergroup_id) => {
  const label = !usergroups[slack_usergroup_id] ? "" : `|@${usergroups[slack_usergroup_id].handle}`;
 ...
 *                   "Kahvinkittaajat" EDIT: removed the handle, because we don't
 ...
 *   }
 */
const getUsergroupsForChannels = (channel_ids) =>
  channel_ids.map((slack_channel_id) => ({
...
const processUpdateEvent = (response) => {
  ...
  if (oldUg && newUg.date_update < oldUg.date_update) {
    console.log(
      `subteam_updated: data is older than pre-existing data for usergroup ${newUg.id}, ignoring event`,
    );
    return false;
  }
...
const processMembersChangedEvent = (response) => {
  ...
  if (response.date_previous_update !== ug.date_update) {
    console.log(
      `subteam_members_changed: update time mismatch for usergroup ${ug.id}, ignoring data`,
    );
    return false;
  }
