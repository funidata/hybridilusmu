...
describe('Populate from API call', function () {
  ...
  it('usergroups.list with users included', function () {
    assert.equal(usergroups.insertUsergroupsFromAPIListResponse(usergroupsListWithUsersPayload), true)
  ...
  it('usergroups.list without providing users is falsy', function () {
    assert.equal(usergroups.insertUsergroupsFromAPIListResponse(usergroupsListPayload), false)
  })

  it('usergroups.list + usergroups.users.list inserts things correctly', function () {
    assert.equal(usergroups.insertUsergroupsFromAPIListResponse(usergroupsListPayload), false)
    assert.equal(
      usergroups.insertUsergroupUsersFromAPIListResponse(usergroupsUsersListPayload, usergroupsListPayload.usergroups[0].id),
      true
    )
    assert.equal(usergroups.isUserInUsergroup("Umeklu", "Skahvi"), true)
    assert.equal(usergroups.isUserInUsergroup("Umcafee", "Skahvi"), false)
